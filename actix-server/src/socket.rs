...
impl Drop for MioListener {
    fn drop(&mut self) {
        #[cfg(unix)]
        {
            // The UDS path should only be cleaned up once the listener is dropped.
            // `deregister` is used for pause/resume and error backoff and must not unlink.
            if let MioListener::Uds(lst) = self {
